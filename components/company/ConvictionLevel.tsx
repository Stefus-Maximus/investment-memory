...
export function ConvictionLevel({
  ...
  return (
    <>
      <div className="flex shrink-0 flex-col items-start gap-1 rounded-xl bg-blue-50 px-3 py-2">
        <span className="text-[10px] font-medium uppercase tracking-wide text-blue-400">
          Overtuiging
        </span>
        <span className="text-sm font-semibold text-slate-900">{convictionLabel(conviction)}</span>
        <div className="flex items-center gap-1.5">
          <span className="flex items-center gap-1" aria-hidden="true">
            {Array.from({ length: DOT_COUNT }, (_, i) => i + 1).map((level) => (
              <span
                key={level}
                className={`h-1.5 w-1.5 rounded-full ${level <= filled ? 'bg-blue-600' : 'bg-blue-200'}`}
              />
            ))}
          </span>
          <button
